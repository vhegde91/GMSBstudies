...
os.system("cd ../;rm GMSBstudies.tar ;tar -cf GMSBstudies.tar *")
#os.system("cd ../../AnalysisTools ; rm AnalysisTools.tar ; git archive master -o AnalysisTools.tar")
os.system("cd ../../AnalysisTools; rm AnalysisTools.tar ; tar -cf AnalysisTools.tar *")

commitHash = subprocess.check_output("cd ../ ; git rev-parse HEAD",shell=True)

#os.system('eos root://cmseos.fnal.gov mkdir /eos/uscms/store/user/vhegde/GMSBstudies/{0}'.format(options.executable))
#os.system("test=`eos root://cmseos.fnal.gov ls /eos/uscms/store/user/vhegde/GMSBstudies/{0} | grep 'No such file or directory'` ; if [ $test == '' ] ; then ;  eos root://cmseos.fnal.gov mkdir /eos/uscms/store/user/vhegde/GMSBstudies/{0} ; fi".format(options.executable))

#os.system('eos root://cmseos.fnal.gov mkdir /eos/uscms/store/user/vhegde/GMSBstudies/{0}/{1}'.format(options.executable,commitHash))
#os.system("test=`eos root://cmseos.fnal.gov ls /eos/uscms/store/user/vhegde/GMSBstudies/{0}/{1} | grep 'No such file or directory'` ; if [ $test == '' ] ; then ; eos root://cmseos.fnal.gov mkdir /eos/uscms/store/user/vhegde/GMSBstudies/{0}/{1} ; fi".format(options.executable,commitHash))
